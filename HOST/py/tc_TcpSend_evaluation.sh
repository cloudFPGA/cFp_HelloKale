...
# This script evaluates a tc_*TCP_SEND*.log file. It writes the summary of analysis for  
# TCP_SEND test
...
#    echo "new file is going to be named ${file/.*}_success.txt"
    ...
    echo "usage: tc_TcpSend_evaluation.sh filename.log"
    exit
fi
...
then
	...
	then
	   ...
	   echo "ext = ${ext}"
		...
		echo "usage: tc_TcpSend_evaluation.sh filename.log"
		echo "the file has to be a tc_*TCP_SEND*.log file"
	fi
fi
