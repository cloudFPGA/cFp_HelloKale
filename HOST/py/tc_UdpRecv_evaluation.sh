...
# This script evaluates a tc_*UDP_RECV*.log file. It writes the summary of analysis for 
# UDP_RECV tests 
...
#    echo "new file is going to be named ${file/.*}_success.txt"
    ...
    echo "usage: tc_UdpRecv_evaluation.sh filename.log"
    exit
fi
...
then
	...
	then
	   ...
	   echo "ext = ${ext}"
		...
		echo "usage: tc_UdpRecv_evaluation.sh filename.log"
		echo "the file has to be a tc_*UDP_RECV*.log file"
		exit 1
	fi
fi
